...
extern "C"
{
                         ...
                         uint numParticles)
    {
        ...
        thrust::device_ptr<float4> d_oldPos4((float4 *)oldPos);

        thrust::for_each(
            thrust::make_zip_iterator(thrust::make_tuple(d_pos4, d_vel4, d_oldPos4)),
            thrust::make_zip_iterator(thrust::make_tuple(d_pos4+numParticles, d_vel4+numParticles, d_oldPos4+numParticles)),
    				...
    				uint   numParticles)
    {
		...
		// execute the kernel
		lambdaD<<< numBlocks, numThreads >>>((float *)lambda,
											 (float4 *)sortedPos,
											 cellStart,
											 cellEnd,
											 numParticles);
					...
					uint   numParticles)
	{
		...
		// execute the kernel
		deltaP_D<<< numBlocks, numThreads >>>((float *)lambda,
											  (float3 *)delta_p,
											  (float4 *)sortedPos,
											  cellStart,
											  cellEnd,
                 ...
                 uint   numCells)
    {
        ...
        // execute the kernel
        collideD<<< numBlocks, numThreads >>>(delta_t,
        									  (float4 *)newVel,
                                              (float4 *)sortedPos,
                                              (float4 *)sortedVel,
                                              cellStart,
    ...
    void update_position(float *sortedPos, float *delta_p, uint numParticles)
    {
		...
		// execute the kernel
		update_positionD<<< numBlocks, numThreads >>>((float4 *)sortedPos,
													  (float3 *)delta_p,
													  numParticles);
